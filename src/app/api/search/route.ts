...
export async function GET(req: NextRequest): Promise<NextResponse<SearchResult>> {
    ...
    // and repeat lookups are never throttled. spendIsbndbToken applies the
    // client's own allowance and then the site-wide budget, in that order --
    // shared with both ISBN lookup paths so the three cannot drift apart.
    const limit = await spendIsbndbToken(client);
    if (!limit.allowed) {
